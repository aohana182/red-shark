# Spec: savage-tongue — Local Windows Dictation App
...
**Success looks like:** hold Caps Lock → speak → release → clean text appears where your cursor was, in under ~2-3 seconds for a typical sentence.
...
- **Windows integration:**
  - Hotkey: **Caps Lock, hold-to-talk**, via a low-level keyboard hook (`WH_KEYBOARD_LL` through `ctypes`/`pywin32`) — chosen because `RegisterHotKey` only fires on a combo press, not hold-duration, and Caps Lock has zero collision with any OS or app global shortcut (verified: all `Win+*` combos are OS-reserved and cannot be overridden; `Ctrl+Alt+*` fires accidentally via AltGr on non-US layouts — both avoided)
...
dictate/
  ...
  hotkey.py          → Caps Lock hold-to-talk keyboard hook
...
## Success Criteria
- [ ] Hold Caps Lock → speak → release → correct text appears at cursor in the focused app
...
- [ ] Caps Lock's normal toggle behavior does not interfere with or get triggered by the hook
...
## Open Questions
None outstanding — resolved via conversation on 2026-08-15 (project name, hotkey, model sizes).

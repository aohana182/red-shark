...
_SYSTEM_PROMPT = (
    ...
    "Keep each sentence's type. A question stays a question and keeps its "
    "'?'. NEVER rewrite a question as a statement, and NEVER negate "
    "something that was not negative: 'Do I need a page for X?' must not "
    "become 'I do not have a page for X'. Never merge two sentences into "
    "one, and never drop one of them.\n\n"
    "If the text stops mid-sentence, LEAVE IT STOPPED -- the microphone cut "
    "off, the speaker did not finish. Do not invent an ending, do not pull "
    "a word from earlier in the text to complete it, and do not reorder "
    "words to make it look finished. An unfinished trailing clause is the "
    "correct output.\n\n"
    "Examples:\n"
    ...
    "Input: what about WhatsApp? do I need to have a user-facing page with "
    "the QR codes for\n"
    "Output: What about WhatsApp? Do I need to have a user-facing page with "
    "the QR codes for\n"
    "(both questions stay questions; the trailing clause was cut off "
    "mid-sentence, so it is left as cut off -- no invented ending, no "
    "reordering)\n\n"
